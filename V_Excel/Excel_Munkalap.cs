    ...
    public static partial class Module_Excel
    {
        ...
        public static void Szűrés(string munkalap, string oszloptól, string oszlopig, int sorig, int sortól = 1)
        {
            try
            {
                ...
                object result = myRange.AutoFilter();
            }
            catch (Exception ex)
            {
                HibaNapló.Log(ex.Message, $"Szűrés(munkalap {munkalap}, oszloptól {oszloptól}, oszlopig {oszlopig}, sor {sor})", ex.StackTrace, ex.Source, ex.HResult);
                MessageBox.Show(ex.Message + "\n\n a hiba naplózásra került.", "A program hibára futott", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    }
