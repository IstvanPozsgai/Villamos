...
public static class GombLathatosagKezelo
{
    ...
    private static void GombokSzemélyes(Form form, List<Adat_Bejelentkezés_Gombok> gombok, int OldalId, string Telephely = "")
    {
        try
        {
            ...
            // Lekérjük az aktuális oldal ID-ját
            Adat_Belépés_Oldalak AdatOldal = Program.PostásOldalak.Where(o => o.OldalId == OldalId).FirstOrDefault();
            if (AdatOldal == null) return;
    ...
    ///      Szerszám/Helyiség-hez
    /// </summary>
    /// <param name="AblakNév"></param>
    /// <returns></returns>
    public static List<string> Telephelyek(string AblakNév, int OldalId)
    {
        List<string> Válasz = new List<string>();
        try
        {
