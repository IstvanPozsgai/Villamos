...
namespace Villamos.Villamos_Ablakok._4_Nyilvántartások.Kerékeszterga
{
    public partial class Ablak_Eszterga_Karbantartás_Módosít : Form
    {
        ...
        private List<Adat_Eszterga_Üzemóra> AdatokUzemora;
        ...
        private void TáblaListázásMűvelet()
        {
            try
            {
                ...
                AdatokUzemora = Funkció.Eszterga_ÜzemóraFeltölt();
                AdatTábla.Clear();
                ...
                foreach (Adat_Eszterga_Műveletek rekord in AdatokMűvelet)
                {
                    ...
                    Adat_Eszterga_Üzemóra uzemoraRekord = AdatokUzemora
        ...
        private int TxtBxEllenőrzés(bool újRekord = false)
        {
            ...
            if (true)
            {
                ...
                if (Egyseg == "Üzemóra" || Egyseg == "Bekövetkezés")
                {
                    ...
                    {
                        long aktualisUzemora = AdatokUzemora.Count > 0 ? AdatokUzemora.Max(u => u.Üzemóra) : 0;
        ...
        private bool ModositasEll(bool Muvelet = false)
        {
            ...
            {
                AdatokUzemora = Funkció.Eszterga_ÜzemóraFeltölt();
                Adat_Eszterga_Üzemóra rekord = AdatokUzemora
        ...
        private Adat_Eszterga_Üzemóra KeresÜzemóra(long uzemora, DateTime datum, EsztergaEgyseg egyseg)
        {
            ...
            if (egyseg == EsztergaEgyseg.Üzemóra)
                return AdatokUzemora.FirstOrDefault(u => u.Üzemóra == uzemora && !u.Státus);
        ...
        private void UzemoraKiolvasasEsBeiras(DateTime datum, TextBox txt)
        {
            ...
            if (uzemoraRekord != null)
            {
                txt.Text = uzemoraRekord.Üzemóra.ToStrTrim();
                txt.Enabled = false;
            }
            else
            {
        ...
        private void Btn_ÚjFelvétel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Adat_Eszterga_Üzemóra legutolsóÜzemóra = (from a in AdatokUzemora
                                                          where !a.Státus
                                                          orderby a.Dátum descending
                                                          select a).FirstOrDefault();
        ...
        {
            if (TáblaUtólagMűvelet.Focused && TáblaUtólagMűvelet.SelectedRows.Count == 1)
            {
                TáblaNapló.ClearSelection();
        ...
        private void BttnUtolag_Modosit_Click(object sender, EventArgs e)
        {
            try
            {
                if (TáblaUtólagMűvelet.SelectedRows.Count == 0 && TáblaNapló.SelectedRows.Count == 0)
                ...
                if (TáblaUtólagMűvelet.SelectedRows.Count != 0)
                    UjUtolagosNaplozas();
                else if (TáblaNapló.SelectedRows.Count != 0 )
                    NaploModositas();
        ...
        private void NaploModositas()
        {
            ...
            foreach (DataGridViewRow sor in TáblaNapló.SelectedRows)
            {
                ...
                Adat_Eszterga_Műveletek_Napló modosított = new Adat_Eszterga_Műveletek_Napló(
                    id,
                    eredeti.Művelet,
                ...
                KézNapló.Update(modosított, eredetiDatum);
            }
        }
        ...
        private void UjUtolagosNaplozas()
        {
            ...
            foreach (DataGridViewRow sor in TáblaUtólagMűvelet.SelectedRows)
            {
                int id = sor.Cells[0].Value.ToÉrt_Int();
                Adat_Eszterga_Műveletek rekord = AdatokMűvelet.FirstOrDefault(a => a.ID == id)
                ...
                bool VanE = AdatokNapló.Any(a => a.ID == id && a.Utolsó_Dátum.Date == datum.Date);
                ...
                Adat_Eszterga_Műveletek_Napló adat = new Adat_Eszterga_Műveletek_Napló(
                    id,
                    muvelet,
                    mennyiNap,
                    mennyiÓra,
        ...
        {
            if (TáblaNapló.Focused && TáblaNapló.SelectedRows.Count == 1)
            {
                TáblaUtólagMűvelet.ClearSelection();

                DataGridViewRow sor = TáblaNapló.SelectedRows[0];
