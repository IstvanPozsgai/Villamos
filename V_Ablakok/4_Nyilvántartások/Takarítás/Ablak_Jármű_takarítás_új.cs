...
#pragma warning restore IDE0044 // Add readonly modifier
        ...
        public void SzerelvényListaElő()
        {
            try
            {
        ...
        public void ÜtemLista()
        {
            try
            {
                ...
                AdatokÜtem = KézÜtem.Lista_Adat(hely, jelszó, szöveg);
            }
            catch (HibásBevittAdat ex)
            {
                MessageBox.Show(ex.Message, "Információ", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void HelyekEllenőrzése()
        {
            try
            {
                ...
                string hely = $@"{Application.StartupPath}\Főmérnökség\Adatok\Takarítás\";
                if (!System.IO.Directory.Exists(hely)) System.IO.Directory.CreateDirectory(hely);

                hely += "Jármű_takarítás.mdb";
                if (!Exists(hely)) Adatbázis_Létrehozás.Járműtakarító_Főmérnök_tábla(hely);

                hely = $@"{Application.StartupPath}\Főmérnökség\Adatok\Takarítás\Jármű_takarítás_Napló_{DateTime.Now.Year}.mdb";
                if (!Exists(hely)) Adatbázis_Létrehozás.Járműtakarító_Főmérnök_Napló(hely);


                hely = $@"{Application.StartupPath}\{Cmbtelephely.Text.Trim()}\Adatok\Takarítás\";
                if (!System.IO.Directory.Exists(hely)) System.IO.Directory.CreateDirectory(hely);

                hely += $"Takarítás_{DateTime.Now.Year}.mdb";
        ...
        private void JK_törlés_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int sorszám = 0; sorszám < JK_List.SelectedItems.Count; sorszám++)
                {
                    ...
                    TakTeljesítésLista(JDátum.Value);
        ...
        private void J1Mentés_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (int.TryParse(J1Megfelelő.Text, out int result) == false) return;
                if (int.TryParse(J1NemMegfelelő.Text, out int result1) == false) return;
                ...
                if (AdatJ1 != null)
                {
                    szöveg = "UPDATE j1adatok  SET ";
                    szöveg += "J1megfelelő='" + J1Megfelelő.Text.Trim() + "', ";
                    szöveg += "J1nemmegfelelő='" + J1NemMegfelelő.Text.Trim() + "' ";
                    szöveg += " WHERE Dátum =#" + JDátum.Value.ToString("M-d-yy") + "#";
                    szöveg += $" And napszak={napszak}";
                    szöveg = szöveg + " And típus='" + J1Típus.Text.Trim() + "'";
                }
                else
                {
                    szöveg = "INSERT INTO j1adatok  (dátum, napszak, típus, J1megfelelő, J1nemmegfelelő) VALUES (";
                    szöveg += $"'{JDátum.Value:yyyy.MM.dd}', ";
                    szöveg += $"{napszak}, ";
                    szöveg += $"'{J1Típus.Text.Trim()}', ";
                    szöveg += $"{J1Megfelelő.Text}, ";
                    szöveg += $"{J1NemMegfelelő.Text}) ";
                }
                MyA.ABMódosítás(hely, jelszó, szöveg);
        ...
        private void Típusfeltöltés()
        {
            try
            {
                ...
                szöveg = "Select * FROM típustábla order by id";
                J1Típus.Items.Clear();
                J1Típus.Items.Add("");
                J1Típus.Items.Clear();
                J1Típus.BeginUpdate();
                J1Típus.Items.AddRange(Függvénygyűjtemény.ComboFeltöltés(hely, jelszó, szöveg, "típus"));
                J1Típus.EndUpdate();
                J1Típus.Refresh();
            }
            catch (HibásBevittAdat ex)
        ...
        private void Button3_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int napszak = 2;
                if (Jnappal.Checked == true) napszak = 1;
                ...
                if (AdatTakTelj != null)
                {
                    szöveg = "UPDATE teljesítés  SET ";
                    szöveg += " státus= 3 ";
                    szöveg += $" where dátum=#{JDátum.Value:MM-dd-yyyy}# ";
                    szöveg += $" and napszak={napszak}";
                    szöveg += " AND státus=1 ";
                    szöveg += $" AND takarítási_fajta='{Opció_lista.Text.Trim()}'";
                    szöveg += $" AND azonosító='{Opció_psz.Text.Trim()}'";
                    MyA.ABMódosítás(hely, jelszó, szöveg);
        ...
        private void Opció_mentés_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (double.TryParse(Opció_terület.Text, out double result) == false) return;
                ...
                {
                    szöveg = "UPDATE Teljesítés SET ";
                    szöveg += " mérték=" + Opció_terület.Text.Replace(",", ".");
                    //szöveg += szöveg1;
                    szöveg += $" WHERE dátum=#{AdatTeljes.Dátum:M-d-yy}#";
                    szöveg += $" and napszak={napszak} ";
                    szöveg += $" and azonosító='{AdatTeljes.Azonosító}'";
                    szöveg += $" and takarítási_fajta='{AdatTeljes.Takarítási_fajta}'";
                    szöveg += $" and státus=1 ";
                }
                ...
                {
                    szöveg = "INSERT INTO Teljesítés (azonosító, takarítási_fajta, dátum, megfelelt1, státus, megfelelt2, pótdátum, napszak, típus,  mérték ) VALUES (";
                    szöveg += $"'{Opció_psz.Text.Trim()}', ";  // azonosító
                    szöveg += $"'{Opció_lista.Text.Trim()}', ";  // takarítási_fajta
                    szöveg += $"'{JDátum.Value:yyyy.MM.dd}', "; // dátum
                    szöveg += " 0, 1, 0, false, "; // megfelelt1, státus, megfelelt2, pótdátum
                    szöveg += $" {napszak}, ";
                    szöveg += $"'{típus.Trim()}', "; // típus
                    szöveg += Opció_terület.Text.Replace(",", ".") + ")";
        ...
        private void Előzmény_lista_takarítás()
        {
            try
            {
                ...
                if (Utolsó_pályaszám.Text.Trim() == "" && Utolsó_telephely.Text.Trim() == "")
                    ...
                    szöveg += $" where azonosító='{Utolsó_pályaszám.Text.Trim()}' order by  Takarítási_fajta asc, dátum desc";

                else if (Utolsó_pályaszám.Text.Trim() == "" && Utolsó_telephely.Text.Trim() != "")
                    szöveg += $" where telephely='{Utolsó_telephely.Text.Trim()}'  order by  Takarítási_fajta asc, dátum desc";

                else
                    szöveg += $" where azonosító='{Utolsó_pályaszám.Text.Trim()}' AND telephely='{Utolsó_telephely.Text.Trim()}'  order by  Takarítási_fajta asc, dátum desc";
        ...
        private void Utolsó_pályaszám_feltöltés()
        {
            ...
            szöveg = "SELECT DISTINCT azonosító FROM takarítások ";
            ...
            Utolsó_pályaszám.BeginUpdate();
            Utolsó_pályaszám.Items.AddRange(MyF.ComboFeltöltés(hely, jelszó, szöveg, "azonosító"));
            Utolsó_pályaszám.EndUpdate();
        ...
        private void Utolsó_takfajta_feltöltés()
        {
            ...
            Utolsó_takarítási_fajta.Items.Add("J2");
            Utolsó_takarítási_fajta.Items.Add("J3");
            Utolsó_takarítási_fajta.Items.Add("J4");
            Utolsó_takarítási_fajta.Items.Add("J5");
            Utolsó_takarítási_fajta.Items.Add("J6");
        }
        ...
        private void Tábla_utolsó_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (Tábla_utolsó.Rows[i].Cells[4].Value.ToÉrt_Bool())

                {
                    Utolsó_státus.Checked = true;
                }
                else
                {
                    Utolsó_státus.Checked = false;
                }
            }
            catch (HibásBevittAdat ex)
            {
        ...
        private void Utolsó_tört_lista()
        {
            try
            {
        ...
        private void Utolsó_módosít_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (AdatTak != null)
                {
                    // módosítás
                    szöveg = "UPDATE takarítások  SET ";
                    szöveg += " dátum='" + Utolsó_dátum.Value.ToString("yyyy.MM.dd") + "', ";
                    szöveg += $" státus={státus} ";
                    szöveg += " WHERE azonosító='" + Utolsó_pályaszám.Text.ToStrTrim() + "' AND ";
                    szöveg += " takarítási_fajta='" + Utolsó_takarítási_fajta.Text.ToStrTrim() + "' AND ";
                    szöveg += " telephely='" + Utolsó_telephely.Text.ToStrTrim() + "'";
                }
                else
                {
                    // új elem
                    szöveg = "INSERT INTO takarítások (azonosító, dátum, Takarítási_fajta, Telephely, státus ) VALUES (";
                    szöveg += "'" + Utolsó_pályaszám.Text.ToStrTrim() + "', ";
                    szöveg += "'" + Utolsó_dátum.Value.ToString("yyyy.MM.dd") + "', ";
                    szöveg += "'" + Utolsó_takarítási_fajta.Text.ToStrTrim() + "', ";
                    szöveg += "'" + Utolsó_telephely.Text.ToStrTrim() + "', ";
                    szöveg += $" {státus}) ";
                }
                MyA.ABMódosítás(hely, jelszó, szöveg);
                Előzmény_lista_takarítás();
        ...
        private void Ütem_takfajta_feltöltés()
        {
            ...
            Ütem_takarítási_fajta.Items.Add("J2");
            Ütem_takarítási_fajta.Items.Add("J3");
            Ütem_takarítási_fajta.Items.Add("J4");
            Ütem_takarítási_fajta.Items.Add("J5");
            Ütem_takarítási_fajta.Items.Add("J6");
        }
        ...
        private void Ütem_pályaszám_feltöltés()
        {
            ...
            szöveg = "SELECT DISTINCT  azonosító FROM ütemező ";
            if (Cmbtelephely.Text.Trim() != "Főmérnökség" || Program.Postás_Vezér)
                szöveg += $"WHERE Telephely='{Cmbtelephely.Text.Trim()}'";
            szöveg += "order by azonosító";

            Ütem_azonosító.Items.Clear();
            Ütem_azonosító.BeginUpdate();
            Ütem_azonosító.Items.AddRange(Függvénygyűjtemény.ComboFeltöltés(hely, jelszó, szöveg, "azonosító"));
            Ütem_azonosító.EndUpdate();
        ...
        private void Listázza_Ütemtervet()
        {
            try
            {
                ...
                if (!Exists(hely))
                    return;
                jelszó = "seprűéslapát";
                szöveg = "SELECT * FROM ütemező";

                Kezelő_Jármű_Takarítás_Ütemező KézÜtem = new Kezelő_Jármű_Takarítás_Ütemező();
                List<Adat_Jármű_Takarítás_Ütemező> AdatokÜtem = KézÜtem.Lista_Adat(hely, jelszó, szöveg);
        ...
        private void Ütem_Tábla_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (Ütem_Tábla.Rows[i].Cells[6].Value.ToÉrt_Bool())
                {
                    Ütem_státus.Checked = true;
                }
                else
                {
        ...
        private void Ütem_Rögzítés_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (int.TryParse(Ütem_növekmény.Text, out int result) == false) return;
                ...
                {
                    // Módosítás
                    szöveg = "UPDATE ütemező  SET ";
                    szöveg += "Kezdő_dátum='" + Ütem_kezdődátum.Value.ToString("yyyy.MM.dd") + "', ";  // Kezdő_dátum
                    szöveg += "növekmény=" + Ütem_növekmény.Text.Replace(",", ".") + ", ";  // növekmény
                    szöveg += "Mérték='" + Ütem_mérték.Text.ToStrTrim() + "', "; // Mérték
                    szöveg += $" státus={státus}";
                    szöveg += " WHERE  azonosító='" + Ütem_azonosító.Text.ToStrTrim() + "' AND ";
                    szöveg += " takarítási_fajta='" + Ütem_takarítási_fajta.Text.ToStrTrim() + "' AND ";
                    szöveg += " telephely='" + Ütem_telephely.Text.ToStrTrim() + "'";
                }
                else
                {
                    // új elem
                    szöveg = "INSERT INTO ütemező (azonosító, Kezdő_dátum, növekmény, Mérték, Takarítási_fajta, Telephely, státus) VALUES (";
                    szöveg += "'" + Ütem_azonosító.Text.ToStrTrim() + "', ";  // azonosító
                    szöveg += "'" + Ütem_kezdődátum.Value.ToString("yyyy.MM.dd") + "', ";  // Kezdő_dátum
                    szöveg += Ütem_növekmény.Text.Replace(",", ".") + ", ";  // növekmény
                    szöveg += "'" + Ütem_mérték.Text.ToStrTrim() + "', "; // Mérték
                    szöveg += "'" + Ütem_takarítási_fajta.Text.ToStrTrim() + "', "; // Takarítási_fajta
                    szöveg += "'" + Ütem_telephely.Text.ToStrTrim() + "', "; // Telephely
                    szöveg += $"{státus})";
                }
                MyA.ABMódosítás(hely, jelszó, szöveg);
                Listázza_Ütemtervet();
        ...
        private void J1Listázás()
        {
            ...
            {
                string hely, jelszó, szöveg;
                hely = $@"{Application.StartupPath}\{Cmbtelephely.Text.Trim()}\Adatok\Takarítás\Takarítás_" + ListaDátum.Value.ToString("yyyy") + ".mdb";
                if (!Exists(hely))
                    return;
                jelszó = "seprűéslapát";
        ...
        private void LétszámListázás()
        {
            try
            {
                ...
                szöveg = "SELECT * FROM létszám";
                DateTime listadátum = DateTime.Parse(ListaDátum.Value.ToString("MM") + "-1-" + ListaDátum.Value.ToString("yyyy"));
                int napszak = 1;
                if (RadioButton1.Checked != true)
                    napszak = 2;
                szöveg += " order by dátum";

                Kezelő_Jármű_Takarítás_Létszám KézLétsz = new Kezelő_Jármű_Takarítás_Létszám();
                List<Adat_Jármű_Takarítás_Létszám> AdatLétsz = KézLétsz.Lista_Adat(hely, jelszó, szöveg);
        ...
        private void Létszám_excel()
        {
            try
            {
                ...
                AdatLétsz.Clear();
                AdatLétsz = KézLétsz.Lista_Adat(hely, jelszó, szöveg);
        ...
        private void J2_excel(string munkalap)
        {
            try
            {
                ...
                TakTeljesítésLista(ListaDátum.Value);
        ...
        private void Összesítő_eredmény()
        {
            try
            {
                ...
                hely = $@"{Application.StartupPath}\{Cmbtelephely.Text.Trim()}\Adatok\Takarítás\Takarítás_{ListaDátum.Value.Year}.mdb";
                jelszó = "seprűéslapát";
        ...
        private void Button4_Click(object sender, EventArgs e)
        {
            ...
            {
                string hely, jelszó, szöveg;
                hely = Application.StartupPath + @"\Főmérnökség\Adatok\Takarítás\Jármű_Takarítás_Napló_" + ListaDátum.Value.ToString("yyyy") + ".mdb";
                if (!Exists(hely))
                    return;
                jelszó = "seprűéslapát";

                szöveg = "SELECT * FROM Takarítások_napló ";
                ...
                Kezelő_Jármű_Takarítás_Napló KézNapló = new Kezelő_Jármű_Takarítás_Napló();
                List<Adat_Jármű_Takarítás_Napló> AdatokNapló = KézNapló.Lista_Adat(hely, jelszó, szöveg);
        ...
        private void Btn_vezénylésexcel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Ütemezés_lista.Items.Count == 0)
                    return;
        ...
        private void Gepi_palyaszam_feltoltes()
        {
            ...
            szöveg = "SELECT DISTINCT azonosító FROM Állománytábla WHERE Törölt=false ";
            if (Cmbtelephely.Text.Trim() != "Főmérnökség" || Program.Postás_Vezér)
                szöveg += $"AND üzem='{Cmbtelephely.Text.Trim()}'";
            szöveg += "order by azonosító";

            Gepi_pályaszám.Items.Clear();
            Gepi_pályaszám.BeginUpdate();
            Gepi_pályaszám.Items.AddRange(MyF.ComboFeltöltés(hely, jelszó, szöveg, "azonosító"));
            Gepi_pályaszám.EndUpdate();
        ...
        private void Telephelyek()
        {
            ...
            Tel_TB.BeginUpdate();
            Tel_TB.Items.Add("");
            Tel_TB.Items.AddRange(Függvénygyűjtemény.ComboFeltöltés(hely, jelszó, szöveg, "telephelynév"));
            Tel_TB.EndUpdate();
            Tel_TB.Refresh();
        }
        ...
        private void Gepi_lista_Napló()
        {
            try
            {
                ...
                string hely = Application.StartupPath + @"\Főmérnökség\Adatok\Takarítás\Jármű_Takarítás_napló_" + Gepi_datum.Value.ToString("yyyy") + ".mdb";
                string jelszó = "seprűéslapát";

                Kezelő_Jármű_Takarítás_Napló KézNapló = new Kezelő_Jármű_Takarítás_Napló();
                List<Adat_Jármű_Takarítás_Napló> AdatokNapló;

                string szöveg = "SELECT * FROM takarítások_napló WHERE Takarítási_fajta='Gépi' ";
                ...
                AdatokNapló = KézNapló.Lista_Adat(hely, jelszó, szöveg);
        ...
        private void GépiTípusCmbFeltölt()
        {
            ...
            Kezelő_Jármű_Takarítás_típus KézTíp = new Kezelő_Jármű_Takarítás_típus();
        ...
        private void Gepi_excel_Click(object sender, EventArgs e)
        {
            try
            {
                if (Gepi_dataGrid_.Rows.Count <= 0)
                    return;
                string fájlexc;
        ...
        private void Gepi_rogzit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (AdatTakarítások != null)
                {
                    // módosítás
                    szöveg = "UPDATE takarítások  SET ";
                    szöveg += " dátum='" + Gepi_datum.Value.ToString("yyyy.MM.dd") + "', ";
                    szöveg += $" státus={státus} ";
                    szöveg += $" WHERE azonosító='{Gepi_pályaszám.Text.Trim()}' AND takarítási_fajta='Gépi' AND ";
                ...
                else
                {
                    // új elem
                    szöveg = "INSERT INTO takarítások (azonosító, dátum, Takarítási_fajta, Telephely, státus ) VALUES (";
                    szöveg += "'" + Gepi_pályaszám.Text.Trim() + "', ";
                    szöveg += "'" + Gepi_datum.Value.ToString("yyyy.MM.dd") + "', ";
                    szöveg += "'Gépi', ";
                    szöveg += "'" + Cmbtelephely.Text.ToStrTrim() + "', ";
                    szöveg += $" {státus}) ";
        ...
        private void JárműHibaLista()
        {
            try
            {
                ...
                AdatokJárműHiba = KézJárműHiba.Lista_adatok(hely, jelszó, szöveg);
            }
            catch (HibásBevittAdat ex)
            {
                MessageBox.Show(ex.Message, "Információ", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void TakTeljesítésLista(DateTime Dátum)
        {
            try
            {
                AdatokTelj.Clear();
        ...
        private void J1Lista(DateTime Dátum)
        {
            try
            {
                AdatokJ1.Clear();
        ...
        private void JelenlétLista()
        {
            try
            {
                ...
                AdatokJelen = KézJelen.Lista_Adatok(hely, jelszó, szöveg);
            }
            catch (HibásBevittAdat ex)
            {
                MessageBox.Show(ex.Message, "Információ", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void ÁrakLista()
        {
            try
            {
                string hely = $@"{Application.StartupPath}\főmérnökség\adatok\takarítás\Jármű_Takarítás.mdb";
                string jelszó = "seprűéslapát";
                string szöveg = "SELECT * FROM árak";
                AdatokÁrak.Clear();
                AdatokÁrak = KézÁr.Lista_Adatok(hely, jelszó, szöveg);
